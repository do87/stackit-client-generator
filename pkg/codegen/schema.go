...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	if t == "" || t == "object" {
		...
		if len(path) > 1 { // handle additional type only on non-toplevel types
			...
			// cause conflicts - see https://dev.azure.com/schwarzit/schwarzit.odj.core/_git/stackit-client-generator.git/issues/832
			var typeName string
...
func oapiSchemaToGoType(schema *openapi3.Schema, path []string, outSchema *Schema) error {
	...
	case "string":
		...
		case "email":
			outSchema.GoType = "openapiTypes.Email"
		case "date":
			outSchema.GoType = "openapiTypes.Date"
		case "date-time":
		...
		case "uuid":
			outSchema.GoType = "openapiTypes.UUID"
		case "binary":
			outSchema.GoType = "openapiTypes.File"
		default:
